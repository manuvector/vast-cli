...
def create_instance(offer_id):
    ...
    print("==> Waiting for instance to be ready after creation...")
    for _ in range(30):
        arr = run_json(["vastai", "show", "instances", "--raw"])
        rec = next((x for x in arr if str(x.get("id")) == str(instance_id)), None)
        if rec and rec.get("actual_status") == "running":
            print("    Instance is running.")
            break
        time.sleep(5)
    else:
        print("Timed out waiting for instance to be running.", file=sys.stderr)
        sys.exit(1)
    return instance_id
...
def find_existing_instance():
    arr = run_json(["vastai", "show", "instances", "--raw"])
    for rec in arr:
        if rec.get("actual_status") in ("running", "stopped", "paused"):
            return str(rec.get("id")), rec.get("actual_status")
    return None, None

def turn_on_instance(instance_id):
    print(f"==> Turning on instance {instance_id}")
    run(["vastai", "start", "instance", str(instance_id)])
...
def wait_for_ssh(instance_id):
    ...
    for _ in range(120):
        ...
        if host and port:
            ...
            print("==> SSH command:")
            print(ssh_cmd)
            os.execvp("ssh", ["ssh", "-o", "IdentitiesOnly=yes", "-o", "StrictHostKeyChecking=accept-new", "-i", KEY_PATH, "-p", str(port), f"root@{host}"])
            sys.exit(0)
        time.sleep(5)
...
def main():
    ...
    pubkey = ensure_key()
    register_key(pubkey)
    instance_id, status = find_existing_instance()
    if instance_id:
        print(f"==> Found existing instance: {instance_id} (status: {status})")
        if status != "running":
            turn_on_instance(instance_id)
            for _ in range(30):
                arr = run_json(["vastai", "show", "instances", "--raw"])
                rec = next((x for x in arr if str(x.get("id")) == str(instance_id)), None)
                if rec and rec.get("actual_status") == "running":
